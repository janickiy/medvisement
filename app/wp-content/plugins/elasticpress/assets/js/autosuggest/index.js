...
function init() {
	...
	const fetchResults = async (input) => {
		...
		if (searchText.length >= 2) {
			...
			/**
			 * Лемматизация запроса Medvisement
			 */
			let response = await fetch('/lemmatize', {
				method: 'POST',
				headers: {
			...
			const searchTextLemmatized = await response.text();
			 ...
			 */
			query = applyFilters('ep.Autosuggest.query', query, searchText, input, searchTextLemmatized);
			query = JSON.stringify(query);

			// fetch the results
			response = await esSearch(query, searchText);
