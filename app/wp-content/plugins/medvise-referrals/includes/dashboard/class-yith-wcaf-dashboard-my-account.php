...
if ( ! class_exists( 'YITH_WCAF_Dashboard_My_Account' ) ) {
	...
	class YITH_WCAF_Dashboard_My_Account extends YITH_WCAF_Abstract_Dashboard {
		...
		public function __construct() {
            ...
            // Если пользователь не является рефералом - отключаем панель управления
            $affiliate = YITH_WCAF_Affiliate_Factory::get_affiliate_by_user_id( get_current_user_id() );

			if ( false === $affiliate || ! $affiliate->has_status('enabled') ) {
				return false;
			}
		...
		public function get_dashboard_base_url( $endpoint = '', $value = '' ) {
			 ...
			 */
			return apply_filters( 'yith_wcaf_dashboard_base_url', home_url() );
		}
		...
		public function add_menu_items( $menu_items ) {
			$account_position = array_search( 'edit-account', array_keys($menu_items) ) + 1;

			// adds Affiliate Dashboard endpoint.
			$menu_items = array_slice( $menu_items, 0, $account_position, true ) +
			              [self::$default_endpoint => _x( 'Affiliate Dashboard', '[FRONTEND] My Account menu item', 'yith-woocommerce-affiliates' ) ] +
			              array_slice( $menu_items, $account_position, count( $menu_items ), true );
