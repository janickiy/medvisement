...
if ( ! class_exists( 'YITH_WCAF_Orders' ) ) {
	...
	class YITH_WCAF_Orders {
		...
		public function create_commissions( $order_id, $token, $token_origin = 'undefined' ) {
			// Используется функция из YITH_WCAF_Orders_Premium, удалено чтобы не было путаницы
			return false;
		}
		...
		public function status_changed( $order_id, $old_status, $new_status ) {
			...
			// Время привязки
			$partner_time = (int) get_user_meta( $order->get_user_id(), 'partner_time', true );
			$current_time = time();

			// Комиссия встает в оплату
			if ( 'pending' === $commission_status ) {
				// Получаем заказы пользователя оплаченные
				...
				// Прошло 7 дней - комиссию не создаем
				if ( count( $paid_orders ) === 1 && $current_time > ( $partner_time + 60 * 60 * 24 * 7 ) ) {
					// Удаляем информацию в заказе
					$order->delete_meta_data( '_yith_wcaf_referral' );
					$order->save();
					return;
				}

				// Комиссию можно получать в течении 365 дней с момента привязки
				if ( $partner_time != '' && ( $partner_time + 31536000 ) < $current_time ) {
					// Удаляем информацию в заказе
					$order->delete_meta_data( '_yith_wcaf_referral' );
					$order->save();
					return;
				...
				// Смотрим реферала указанного в заказе, если нужно обновляем
				// todo не стал делать, нужно дописать логи об изменениях. Пусть в заказе будет виден оригинальный реферал

				// Если это первый заказ пользователя, обновляем время привязки
				if ( count($paid_orders) === 1 ) {
					update_user_meta( $order->get_user_id(), 'partner_time', $current_time );
				}
			}
			...
			foreach ( $commissions as $commission ) {
				...
				$affiliate_user      = $affiliate ? $affiliate->get_user() : false;

				if ( $affiliate_user === false ) {
					continue;
				}

				// Меняем аффилята на актуального - если коммиссия на другого создана
				if ( $partner_id !== $affiliate_user->ID ) {
					$new_affiliate = YITH_WCAF_Affiliate_Factory::get_affiliate_by_user_id( $partner_id );

					if ( $new_affiliate ) {
						$commission->set_affiliate_id( $new_affiliate->get_id() );
						$commission->save();
					} else {
						// Не найден афилят - удаляем
