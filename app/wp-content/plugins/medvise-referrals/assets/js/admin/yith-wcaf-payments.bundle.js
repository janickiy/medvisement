...
;// CONCATENATED MODULE: ./assets/js/admin/src/modules/yith-wcaf-admin-notes.js
...
var YITH_WCAF_Admin_Notes = /*#__PURE__*/function () {
  function YITH_WCAF_Admin_Notes($container, objectType, objectID) {
    _classCallCheck(this, YITH_WCAF_Admin_Notes);
    // container element.
    defineProperty_defineProperty(this, "$container", void 0);
    // object type
    defineProperty_defineProperty(this, "objectType", void 0);
    // object ID
    defineProperty_defineProperty(this, "objectID", void 0);
    ...
    this.$notesList = this.$container.find('.order_notes');
    this.$addButton = this.$container.find('#add_note');
    this.$noteContent = this.$container.find('#add_order_note');
    this.$emptyMsg = this.$notesList.find('.no_notes');
    this.init();
  }
  _createClass(YITH_WCAF_Admin_Notes, [{
    key: "init",
    value: function init() {
      var self = this;
      this.$container.on('click', '.delete_note', function (ev) {
        ev.preventDefault();
        var $note = external_jQuery_default()(this).closest('.note');
        self.deleteNote($note);
      });
      this.$addButton.on('click', function (ev) {
        ev.preventDefault();
        var content = self.$noteContent.val();
        if (!content) {
          self.$noteContent.parent().addClass('woocommerce-invalid');
        } else {
          self.addNote(content);
        }
      });
      this.$noteContent.on('keyup', function () {
        var content = self.$noteContent.val();
        if (content) {
          self.$noteContent.parent().removeClass('woocommerce-invalid');
        }
      });
    }
  }, {
    ...
    value: function addNote(note) {
      var _this = this;
      ajax.post.call(this.$container, 'add_note', 'add_note', {
        ...
        if (data !== null && data !== void 0 && (_data$data = data.data) !== null && _data$data !== void 0 && _data$data.template) {
          var $newNote = external_jQuery_default()(data.data.template);
          _this.$notesList.prepend($newNote);
          _this.$noteContent.val('');
          _this.$emptyMsg.hide();
          $newNote.get(0).scrollIntoView();
        }
      });
    }
  }, {
    ...
    value: function deleteNote($note) {
      var _this2 = this;
      ajax.post.call($note, 'delete_note', 'delete_note', {
        object_id: this.objectID,
        object_type: this.objectType,
        ...
        $note.remove();
        if (!_this2.$notesList.find('li').not(_this2.$emptyMsg).length) {
          _this2.$emptyMsg.show();
        }
      });
    }
  }]);
  return YITH_WCAF_Admin_Notes;
}();

;// CONCATENATED MODULE: ./assets/js/admin/src/payments.js
...
var YITH_WCAF_Payments = /*#__PURE__*/function (_YITH_WCAF_Admin_Pane) {
  _inherits(YITH_WCAF_Payments, _YITH_WCAF_Admin_Pane);
  var _super = _createSuper(YITH_WCAF_Payments);
  function YITH_WCAF_Payments() {
    var _this;
    _classCallCheck(this, YITH_WCAF_Payments);
    var $containers = external_jQuery_default()('#yith_wcaf_panel_commissions-payments').add('#yith_wcaf_panel_payment_details');
    _this = _super.call(this, $containers);
    _this.initNotes();
    return _this;
  }
  _createClass(YITH_WCAF_Payments, [{
    key: "initNotes",
    value: function initNotes() {
      var $container = external_jQuery_default()('#payment_notes'),
        $objectID = external_jQuery_default()('#payment_id');
      if (!$container.length || !$objectID.length) {
        return;
      }
      new YITH_WCAF_Admin_Notes($container, 'payment', $objectID.val());
    }
  }]);
  return YITH_WCAF_Payments;
}(YITH_WCAF_Admin_Panel);
external_jQuery_default()(function () {
  return new YITH_WCAF_Payments();
});
...
/******/ })()
;
//# sourceMappingURL=yith-wcaf-payments.bundle.js.map