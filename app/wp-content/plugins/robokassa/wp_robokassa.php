...
require_once('medvise_recurring_payments.php');
...
define( 'ROBOKASSA_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
...
function processRobokassaPayment($order_id, $label)
{
    ...
    if (class_exists('WC_Subscriptions_Order')) {
        $order_subscription = wcs_order_contains_subscription($order_id, array( 'parent', 'resubscribe', 'switch', 'renewal' ));

        if ($order_subscription) {
            $recurring = true;
            // Помечаем заказ, как рекуррентный - в дальнейшем будет использоваться как InvoiceID
