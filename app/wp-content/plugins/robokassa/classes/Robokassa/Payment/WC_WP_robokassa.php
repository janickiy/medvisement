...
class WC_WP_robokassa extends \WC_Payment_Gateway {
    ...
    public function process_subscription_payment( $amount, $renewal_order, $retry = true, $previous_error = false ) {
        ...
        $subscription_related_orders = $subscribe->get_related_orders('all');
        ...
        // Если по какой-то причине заказ подписки не найден - берем просто изначальный
        if ( $last_paid_order === null ) {
            $last_paid_order = $subscribe->get_parent();
        }
        ...
        $data = $robokassa->getRecurringPaymentData($order_id, $last_paid_order->get_id(), $amount, $receipt, 'Оплата подписки');
	    ...
	    $response = wp_remote_post('https://auth.robokassa.ru/Merchant/Recurring', array(
            'header' => 'Content-Type: application/x-www-form-urlencoded',
            'method' => 'POST',
            'body' => http_build_query($data)
        ));

	    medroboLogPaymentRequestResponse( $order_id, $response );

	    // Проверяем платеж через 30 минут
	    as_schedule_single_action(
			time() + 1800,
			'medrobo_check_payment_status',
		    [ 'order_id' => $order_id ]
	    );
    }
